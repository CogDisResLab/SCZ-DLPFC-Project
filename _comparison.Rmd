...
data_modeled[["scaled"]] |>
  ...
  write_csv(
    file.path(
      "results",
      "{{run_prefix}}-signal_{{tag}}_{{case}}_{{ctrl}}-{{chip}}.csv"
    )
  )
...
write_csv(
  ...
  file.path("results", "{{run_prefix}}-dpp_{{tag}}_{{case}}_{{ctrl}}-{{chip}}.csv")
)
...
saveRDS(mutiple_krsa_outputs,
  file = file.path(
    "datastore",
    "{{run_prefix}}-multiple_krsa_output_{{tag}}_{{case}}_{{ctrl}}_{{chip}}.RDS"
  )
)
...
AvgZTable2 |>
  write_csv(
    file.path(
      "results",
      "{{run_prefix}}-krsa_table_{{tag}}_{{case}}_{{ctrl}}_{{chip}}.csv"
    )
  ) |>
  select(Kinase, AvgZ) |>
  unique() |>
