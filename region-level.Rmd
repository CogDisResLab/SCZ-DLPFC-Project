...
# qc processing
qc_data_separate <- krsa_qc_steps(raw_data) |>
  mutate(
    Group = case_when(
      SampleName == "Female CT" ~ "F_CTL",
      SampleName == "Male CT" ~ "M_CTL",
      SampleName == "Female SCHZ" ~ "F_SCZ",
      SampleName == "Male SCHZ" ~ "M_SCZ"
    ),
    SampleName = paste0(Group, "_", Barcode)
  )

qc_data_pooled <- qc_data_separate |>
  mutate(
    Group = case_when(
      Group %in% c("F_CTL", "M_CTL") ~ "CTL",
      Group %in% c("F_SCZ", "M_SCZ") ~ "SCZ"
    ),
    SampleName = str_c(SampleName, Group, sep = "_")
  )

qc_data <- bind_rows(qc_data_separate, qc_data_pooled)
