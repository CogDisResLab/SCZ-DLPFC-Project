---
title: "Human DLPFC Kinome Profiling - Region Level"
...
params:
  ...
  signal_file: "kinome_data/region-level/region_KRSA_files_Median_SigmBg_210807100633.txt" # Change the file name to the one you have
  saturation_file: "kinome_data/region-level/region_KRSA_files_Signal_Saturation_210807100633.txt" # Change the file name to the one you have
...
# qc processing
gendered_data <- krsa_qc_steps(raw_data) |>
  mutate(
    Group = case_when(
      SampleName == "Female CT" ~ "CTL_F",
      SampleName == "Female SCHZ" ~ "SCZ_F",
      SampleName == "Male CT" ~ "CTL_M",
      SampleName == "Male SCHZ" ~ "SCZ_M"
    ),
    SampleName = paste0(Group, "_", Barcode)
  )

pooled_data <- krsa_qc_steps(raw_data) |>
  mutate(
    Group = case_when(
      grepl("CT", SampleName) ~ "CTL",
      grepl("SCHZ", SampleName) ~ "SCZ"
    ),
    Barcode = case_when(
      grepl("Female", SampleName) ~ paste0(Barcode, "F"),
      grepl("Male", SampleName) ~ paste0(Barcode, "M")
    ), 
    SampleName = paste0(Group, "_", Barcode)
  )

qc_data <- bind_rows(gendered_data, pooled_data)
...
- `r groups[[2]]` vs `r groups[[1]]`
- `r groups[[4]]` vs `r groups[[3]]`
- `r groups[[1]]` vs `r groups[[3]]`
- `r groups[[2]]` vs `r groups[[4]]`
- Pooled SCZ vs Pooled CTL
...
comparisons <- list(
  ...
  COMP4 = c(groups[[2L]], groups[[4L]]),
  COMP5 = c(groups[[6]], groups[[5]])
)
```
