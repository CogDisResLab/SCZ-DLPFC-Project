# Create Quartile Rankings from KRSA Data
...
files <- list.files("data/KRSA_Reports/", full.names = TRUE)
...
datasets <- files %>%
  ...
  write_csv("results/KRSA_quartile_rankings_all.csv") %>%
  select(-Score, -rank) %>%
  pivot_wider(names_from = dataset,
              values_from = quartile,
              values_fill = 100) %>%
  write_csv("results/KRSA_quartile_rankings_matrix_all.csv") %>%
...
sum_ordered_kinases <- datasets %>%
  ...
  ungroup() %>%
  arrange(Total_Rank) %>%
  write_csv("results/KRSA_quartile_rankings_aggregated_ordered_all.csv")
...
sum_ordered_kinases_male <- datasets %>%
  ...
  ungroup() %>%
  arrange(Total_Rank) %>%
  write_csv("results/KRSA_quartile_rankings_aggregated_ordered_male.csv")
...
sum_ordered_kinases_female <- datasets %>%
  ...
  ungroup() %>%
  arrange(Total_Rank) %>%
  write_csv("results/KRSA_quartile_rankings_aggregated_ordered_female.csv")
