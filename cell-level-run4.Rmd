...
params:
  ...
  signal_file: "kinome_data/cell-level/run4_KRSA_files_Median_SigmBg_210605104306.txt" # Change the file name to the one you have
  saturation_file: "kinome_data/cell-level/run4_KRSA_files_Signal_Saturation_210605104307.txt" # Change the file name to the one you have
...
raw_data <- krsa_read(
  ...
  filter(str_detect(SampleName, fixed("PIM"), negate = TRUE))

# Load the subject pairings
subject_pairs <- read_csv(file.path("kinome_data", "subject_pairs.csv")) |>
  filter(Run == 4L) |>
  mutate(CTL = str_replace(CTL, fixed("832"), "834"))
```
