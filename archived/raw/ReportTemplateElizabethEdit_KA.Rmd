...
```{r libraries&functions, include=FALSE}
 #loading Libraries ----
devtools::install_github("CogDisResLab/KRSA", "v1.0.0")
library(tidyverse)
library(KRSA)
library(gt)
library(knitr)
...
knitr::include_graphics("images/UTCOMLS.jpeg")
```
...
Schizophrenia vs. Control: Run 1, Pair 1
...
# Read crosstab files 
data <- krsa_read("Paired-Raw-Data/run1/_KRSAsfiles_Median_SigmBg_210518210643.txt", "Paired-Raw-Data/run1/_KRSAsfiles_Signal_Saturation_210518210643.txt")

# qc processing
krsa_qc_steps(data) -> data

# Define sample groups
data %>% mutate(Group = str_extract(SampleName, "^\\w{3}")) -> data
...
* Control vs. Schizophrenia (Pair 1.1)
...
samples_ids <- c("CTL_869", "SCZ_737")


krsa_quick_filter(data = data_pw_200, data2 = data_modeled$scaled,
    signal_threshold = 5, r2_threshold = 0.9, samples = samples_ids,
    groups = comparisons$Comp1) -> pep_passed_qc

krsa_group_diff(data_modeled$scaled, comparisons$Comp1,
                samples = samples_ids,
                pep_passed_qc, byChip = T) -> diff_df

krsa_ce_output_peptides(diff_df, totalMeanLFC, "Run1_comp1_peptide_ce_output.txt")


krsa_get_diff(diff_df,totalMeanLFC ,c(0.2,0.25,0.3)) %>% list("meanLFC" = .) -> sigPeps
...
#### Schizophrenia vs. Control (Pair 1.1) 
...
krsa_heatmap(data_modeled$normalized, sigPeps$meanLFC$`0.2`, groups = comparisons$Comp1, samples = samples_ids, scale = "none")
...
krsa_violin_plot(data_modeled$scaled, sigPeps$meanLFC$`0.2`, "Barcode", groups = comparisons$Comp1, samples = samples_ids)
...
krsa_curve_plot(data_pw, sigPeps$meanLFC$`0.2`, groups = comparisons$Comp1, samples = samples_ids)
```
\newpage  
#### Run1 Comp1  
...
# save file
#fin$KRSA_Table %>% write_delim("Run1_comp1_KRSA_Table_02_cutoff.txt", delim = "\t")
#krsa_ce_output_kinases(fin$KRSA_Table, "Run1_comp1_KRSA_Table_02_cutoff_ce_output.txt")
...
krsa_ce_output_kinases(AvgZTable, "Run1_comp1_KRSA_Table_allCutoffs_ce_output.txt")
...
krsa_top_hits(AvgZTable, 1.75) -> kinases_hits_1_75
krsa_top_hits(AvgZTable, 1.5) -> kinases_hits_1_5
...
krsa_reverse_krsa_plot(chipCov, diff_df, kinases_hits_1_5, 0.2, byChip = F)
```
\newpage
...
krsa_ball_model(kinases_hits, AvgZTable, 10, 2.5, 4.8)
