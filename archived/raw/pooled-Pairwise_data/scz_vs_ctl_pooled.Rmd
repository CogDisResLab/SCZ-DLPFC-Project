...
setwd("C:\\Users\\alexw\\Documents\\Kinases\\Scz Runs")
library(KRSA)
library(knitr)
library(tidyverse)
...
# Start by reading each set of data individually
d1 <- krsa_read("SigmBg_R1.txt", "SigSat_R1.txt")
d2 <- krsa_read("SigmBg_R2.txt", "SigSat_R2.txt") 
d3 <- krsa_read("SigmBg_R3.txt", "SigSat_R3.txt")
d4 <- krsa_read("SigmBg_R4.txt", "SigSat_R4.txt")
...
# Define sample groups; this is a bit more complicated than normal. 
#each data set is processed and combined at the end. Need to assign groups based on array. A1/A3 are defined as CTL and A2/A4 are defined as SCZ
...
test4 <- test4%>%
	mutate(Group = "SCZ")
d1 <- rbind(test1, test2, test3, test4) 
...
test4 <- test4%>%
	mutate(Group = "SCZ")
d2 <- rbind(test1, test2, test3, test4)
...
test4 <- test4%>%
	mutate(Group = "SCZ")
d3 <- rbind(test1, test2, test3, test4) 
...
test4 <- test4%>%
	mutate(Group = "SCZ")
d4 <- rbind(test1, test2, test3, test4)

data <- rbind(d1, d2, d3, d4)

data <- data%>%
  mutate(SampleName = paste0(SampleName,"_" ,Barcode))
