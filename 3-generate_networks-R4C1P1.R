# Generate guided network for R4C1P1
...
ctl_subset <- subset_data(stk_data, filtered_peptides$CTL_834, "CTL_834")
scz_subset <- subset_data(stk_data, filtered_peptides$SCZ_446, "SCZ_446")

ctl_model <- make_model(ctl_subset, 1000, threshold = 0)
scz_model <- make_model(scz_subset, 1000, threshold = 0)

saveRDS(list(filtered_peptides, ctl_subset, ctl_model, scz_subset, scz_model), file = "depot/R4C1P1-Model-Data.RDS")
...
pdf("figures/R4C1P1-Comparison.pdf")
compare_kinased_graphs(ctl_kinased, scz_kinased, "CTL_834", "SCZ_446", render = TRUE)
dev.off()
