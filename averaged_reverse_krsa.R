...
comparative_reverse_krsa <- function(kinase, dataset) {
    ...
    p <- g +
        ...
        scale_x_discrete(name = "Subject Pair", limits = pair_order) +
...
combined_plot <- (plots$P38 | plots$JNK | plots$ERK) + plot_layout(guides = "collect", axes = "collect") & theme(legend.position = "bottom")

ggsave("combined_errorbar_plot.png", plot = combined_plot, height = 15L, width = 45L, units = "in", path = "figures")
ggsave("combined_errorbar_plot.svg", plot = combined_plot, height = 15L, width = 45L, units = "in", path = "figures", bg = "transparent")

plots |>
    imap(~ ggsave(str_glue("{.y}_LFC_Errorbar_Plot.png"),
        plot = .x, width = 14L,
        height = 7L, units = "in", path = "figures"
    ))
